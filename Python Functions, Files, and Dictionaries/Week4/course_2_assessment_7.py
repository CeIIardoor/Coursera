...
def checkingIfIn(strx, direction=True, d={'apple': 2, 'pear': 1, 'fruit': 19, 'orange': 5, 'banana': 3, 'grapes': 2, 'watermelon': 7}):
    if direction:
        return strx in d
    else:
        return strx not in d
...
c_false = checkingIfIn('lmao')


# Call the fucntion so that it returns True and assign it to the variable c_true

c_true = checkingIfIn('lmao', False)
...
param_check = checkingIfIn('grapes', True, {'apple': 2, 'pear': 1, 'fruit': 19, 'orange': 5, 'banana': 3, 'grapes': 8, 'watermelon': 7})