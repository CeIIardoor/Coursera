...
third = []
third.append([t[2] for t in l_of_l])
...
t = [athletes[i][j] for i in range(len(athletes)) for j in range(len(athletes[i])) if 't' in athletes[i][j]]
other = [athletes[i][j] for i in range(len(athletes)) for j in range(len(athletes[i])) if 't' not in athletes[i][j]]
